...
-- de 2 horas para el inicio de la clase.
--
...
-- función reservar_clase ya creadas y ejecutadas) y sustituye por completo a
-- la función cancelar_reserva() de sql/003_cancelaciones.sql (CREATE OR
-- REPLACE, misma firma, no hace falta DROP).
-- ============================================================================
...
begin
  ...
  -- 6b. Barrera real: no se puede cancelar si faltan menos de 2 horas para el inicio.
  if (v_clase_inicio - v_ahora_local) < interval '2 hours' then
    raise exception 'No se puede cancelar con menos de 2 horas de antelacion.' using errcode = 'P0003';
  end if;
